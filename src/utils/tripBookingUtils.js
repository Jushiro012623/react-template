...
export const isFormValid = (option, value ,initialValue) => {
    if (option === 1) {
      ...
      return value.data?.vehicle_type && value.data?.plate_number && value.data?.weight && value.data?.item_name && value.data?.cargo_description && value.data?.quantity && value.data?.additional && value.data?.discount
    }
  };